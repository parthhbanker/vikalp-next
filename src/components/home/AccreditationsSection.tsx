...
function AccreditationBadge({ icon, title, subtitle, year }: AccreditationBadgeProps) {
  return (
    <div className="group relative bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 p-4 sm:p-6 border-2 border-brand/20 hover:border-brand/50">
      {/* Badge Icon */}
      <div className="flex flex-col items-center text-center">
        <div className="relative mb-4">
          ...
          <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-br from-brand to-brand/70 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300">
            <div className="text-white [&>svg]:w-7 [&>svg]:h-7 sm:[&>svg]:w-9 sm:[&>svg]:h-9">{icon}</div>
          </div>
          {/* Checkmark indicator */}
